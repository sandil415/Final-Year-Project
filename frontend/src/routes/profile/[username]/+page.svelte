        ...
        <div class="flex-1 min-w-0">
          ...
          {#if isBusiness && (user.businessPhone || user.businessAddress)}
            <div class="flex flex-wrap gap-4 text-xs text-muted-foreground mb-3">
              {#if user.businessPhone}<span class="flex items-center gap-1.5"><PhoneIcon size={12}/>{user.businessPhone}</span>{/if}
              {#if user.businessAddress}<span class="flex items-center gap-1.5"><MapPinIcon size={12}/>{user.businessAddress}</span>{/if}
            </div>
          {/if}
          <div class="flex flex-wrap gap-2">
            {#if isOwnProfile}
              <button class="border border-border px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-muted transition-colors" on:click={() => goto('/profile/edit')}>Edit profile</button>
              {#if isBusiness}
      ...
      <div class="{isBusiness ? 'lg:grid lg:grid-cols-[1fr_280px]' : ''} lg:gap-6 items-start mt-5">
        ...
        <div class="min-w-0">
          ...
          {#if activeTab === 'posts'}
            {#if posts.length === 0}
              <div class="flex flex-col items-center py-20 text-center">
                <div class="w-14 h-14 rounded-full bg-muted flex items-center justify-center mb-4"><CameraIcon size={28} class="text-muted-foreground"/></div>
          ...
          {:else if activeTab === 'recipes'}
            {#if recipes.length === 0}
              <div class="flex flex-col items-center py-20 text-center">
                <div class="w-14 h-14 rounded-full flex items-center justify-center mb-4" style="background-color:#FF6B3515;"><BookOpenTextIcon size={28} style="color:#FF6B35;" weight="duotone"/></div>
                <p class="font-semibold mb-1">{isOwnProfile ? 'Create your first recipe' : 'No recipes yet'}</p>
                {#if isOwnProfile}<button class="text-sm font-semibold mt-3 hover:opacity-70" style="color:#FF6B35;" on:click={openNewRecipe}>Create recipe →</button>{/if}
              </div>
            {:else}
              <div class="grid gap-3">
                {#each recipes as recipe (recipe.id)}
                  {@const rtags = Array.isArray(recipe.tags) ? recipe.tags : (() => { try { return JSON.parse(recipe.tags || '[]'); } catch { return []; } })()}
                  <div class="group flex bg-card border border-border rounded-2xl overflow-hidden cursor-pointer hover:border-orange-300 dark:hover:border-orange-700 transition-all" on:click={() => viewingRecipe = recipe}>
                    {#if recipe.cover_image}<img src={pb.files.getUrl(recipe, recipe.cover_image)} alt="" class="w-28 object-cover flex-shrink-0"/>
                    {:else}<div class="w-28 flex items-center justify-center flex-shrink-0" style="background:#FF6B3508;"><BookOpenTextIcon size={28} style="color:#FF6B35;opacity:0.3;" weight="duotone"/></div>{/if}
                    <div class="flex-1 p-3 min-w-0 flex flex-col justify-between">
                      <div>
                        ...
                        {#if rtags.length}<div class="flex flex-wrap gap-1 mt-1">{#each rtags.slice(0, 3) as tag}<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{tag}</span>{/each}</div>{/if}
                      </div>
                      {#if isOwnProfile}
                        <div class="flex gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity" on:click|stopPropagation>
                          <button class="flex items-center gap-1 px-2 py-1 rounded-lg text-xs border border-border text-muted-foreground hover:bg-muted" on:click={() => openEditRecipe(recipe)}><PencilSimpleIcon size={11}/>Edit</button>
                          <button class="flex items-center gap-1 px-2 py-1 rounded-lg text-xs border border-red-200 text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20" on:click={() => deleteRecipe(recipe)}><TrashIcon size={11}/>Delete</button>
                        </div>
                      {/if}
                    </div>
                  </div>
          ...
          <div class="hidden lg:block">
            <div class="sticky top-20 bg-card border border-border rounded-2xl overflow-hidden flex flex-col" style="max-height: calc(100vh - 120px);">
              ...
              <div class="flex-1 overflow-y-auto p-2 space-y-3">
                {#if menuItems.length === 0}
                  <div class="flex flex-col items-center py-8 text-center"><div class="text-2xl mb-2">🍽️</div><p class="text-xs text-muted-foreground">{isOwnProfile ? 'Add your first item' : 'No items yet'}</p></div>
                {:else}
                  {#each Object.entries(filteredMenuItems) as [category, items]}
                    <div>
